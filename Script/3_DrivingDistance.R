...
# Project: Geospatial Business Intelligence 101: 
#          Computing the shortest driving distance using R
# Author: Raed K Alotaibi
# Link: https://milospopovic.net/geo-business_intelligence-compute-shortest-driving-distance-in-r/?s=09
...
unzip("Input/popisni_krug-gpkg.zip")

#load census circles
pk <- st_read(paste0(getwd(), "/Input/tmp/data/ready/pk/", "popisni_krug.gpkg"), stringsAsFactors = FALSE) %>% 
  st_transform(4326) %>% 
  st_as_sf()
...
# set motorcar as our preferable mode of transport and different types of highways to be used for weighting
...
# Checking distances with an INF value
summary(b$dist)
...
names(c) <- c("long", "lat", "id") #choose intuitive na


# calculate the shortest aerial path
